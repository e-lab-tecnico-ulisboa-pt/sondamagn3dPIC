#!/bin/bash
clear
echo Starting mag3d Driver

# @ Initializing driver starter script @

INITIAL_HEAP_MEM=64m
MAX_HEAP_MEM=128m

export DEPLOY_DIR="/home/elab/rec-deployment/mag3d"
export DRIVER_BASE_DIR="/home/elab/rec-deployment/mag3d/hardwareserver"
...
export PROCESSINGMANAGER_SYSPROPS="-Drec.processingmanager.threadPool.coresize=2 -Drec.processingmanager.threadPool.maxsize=4 -Drec.processingmanager.thread.idletime=11"
export EXPERIMENT_DRIVER_CLASS="-Dexperiment.driver.class=pt.utl.ist.elab.driver.mag3d.Mag3DDriver"
...
export DRIVER_CLASSPATH=$DRIVER_BASE_DIR/mag3dDriver.jar:$DRIVER_BASE_DIR/ELabHardwareServer.jar:$DRIVER_BASE_DIR/ELabCommon.jar:$DRIVER_BASE_DIR/ReCHardwareServer.jar:$DRIVER_BASE_DIR/ReCCommon.jar
...
echo --------------------------------------------------------------------------------
echo "Additional java args : "
echo --------------------------------------------------------------------------------
echo "LD_LIBRARY_PATH      : $LD_LIBRARY_PATH"
echo --------------------------------------------------------------------------------

cd $DEPLOY_DIR
java $BOOTCLASSPATH  -classpath $RECCLASSPATH:$DRIVER_CLASSPATH:$DRIVER_EXPERIMENT_CLASSPATH $GENERIC_ORB_SYSPROPS $DRIVER_ORB_SYSPROPS $LOG_SYSPROPS $PROCESSINGMANAGER_SYSPROPS $MEM_SYSPROPS $DRIVER_HARWARE_INFO_SYSPROPS $TOOLKIT_SYSPROPS $EXPERIMENT_DRIVER_CLASS $DEBUG com.linkare.rec.impl.driver.FullServerMain &

PID=$!
echo $PID > $DEPLOY_DIR/mag3d.pid
